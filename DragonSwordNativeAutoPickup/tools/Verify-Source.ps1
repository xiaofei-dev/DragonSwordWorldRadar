...
if ($SkipBuild) {
    Write-Host 'DragonSwordNativeAutoPickup 1.3.0 ExperimentalNested source, manifest, configurable-hotkey, mods.txt, and package layout gates passed; core build was explicitly skipped.'
} else {
    & (Join-Path $PSScriptRoot 'Build-Core.ps1')
    Write-Host 'DragonSwordNativeAutoPickup 1.3.0 ExperimentalNested source, manifest, configurable-hotkey, mods.txt, package layout, and core tests passed.'
}
