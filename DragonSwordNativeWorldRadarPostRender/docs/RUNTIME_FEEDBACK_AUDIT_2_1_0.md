...
| Wheel zoom flickers, stalls, offsets, or temporarily removes expanded-map markers | Current-source correction revised, not runtime-accepted. A five-deadline tail at 100/250/500/1,000/1,250 ms takes one fresh observation per due pass; overdue deadlines remain due and advance only one observation per later pass. First four passes are read-only; only the final pass may mutate after retained-parent witness and stable geometry. Equal extents rebase retained hosts; a successful fresh attach establishes a new baseline and permits one later stable extent-change rebuild. | Confirm continuous wheel zoom and repeated window/aspect transitions produce no visible flicker/offset, first-four-pass mutation, repeated rebuild loop, or steady work. Verify the exact parent identity, live extents, stability, and rebuild counters in current-build diagnostics. |
...
| Intermittent gameplay stalls | Static inspection bounds the retained-host path and removes repeated equal-extent atlas rebuilding. A proved extent change may still perform one bounded attach. This is not runtime hitch attribution or frame-time acceptance. | Keep Pickup disabled for the next comparison, capture current-build Radar diagnostics plus external frame times, and separate event-local attach cost from steady gameplay. |
...
4. Confirm the five deadlines take one observation per due pass, the first four
   are read-only, equal extents use retained-host reparent/restack, and one
   extent change produces at most one candidate-bound build/attach.
