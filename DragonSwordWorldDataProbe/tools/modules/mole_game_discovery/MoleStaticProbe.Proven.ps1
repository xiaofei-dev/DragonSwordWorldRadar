...
$outputDir = Join-Path $modDir "mole_static_probe"
$rawDir = Join-Path $outputDir "xml"
$tempDir = Join-Path ([IO.Path]::GetTempPath()) ("DSMoleProbe-" + [Guid]::NewGuid().ToString("N"))
$stagePath = Join-Path $modDir "mole_static_probe_last_stage.txt"
$logPath = Join-Path $modDir "mole_static_probe.log"
$mutexName = "Local\DragonSwordMoleStaticProbe_v100"
...
function Stage([string]$Name, [string]$Details) {
    $text = @(
        ...
        "Version: 1.0.3-integrated-exact-decoder",
...
try {
    ...
    try {
        ...
        $patterns = @(
            '(Mole|Gopher|Groundhog|Marmot|Whack|Rodent|Burrow|Hole)',
            '(MiniGame|Mini_Game|Minigame)',
            '(Challenge|Animal).*Data',
            ...
            'String.*(MiniGame|Challenge|Animal|Mole|Gopher|Marmot)',
            '(GameType|GamePlay|Gameplay).*Data'
        )
        function Rank-Candidate($Entry) {
            $name = $Entry.FullName
            if ($name -match '(Mole|Gopher|Groundhog|Marmot|Whack)') { return 0 }
            if ($name -match '(MiniGame|Mini_Game|Minigame)') { return 1 }
            if ($name -match '(Rodent|Burrow|Hole|Animal)') { return 2 }
            ...
            if ($name -match '(Challenge|GameType|Gameplay|GamePlay)') { return 7 }
            return 10
        }
                ...
                try {
                    ...
                    if ($null -ne $fallback) {
                        ...
                        if ($isCritical) {
                            ...
                            throw (
                                $primaryError +
                                ' | exact compact recovery failed; evidence captured'
                            )
                        }
