...
$outputDir = Join-Path $modDir "treasure_static_fast_probe"
$rawDir = Join-Path $outputDir "xml"
$tempDir = Join-Path ([IO.Path]::GetTempPath()) ("DSTreasureStaticFast-" + [Guid]::NewGuid().ToString("N"))
$stagePath = Join-Path $modDir "treasure_static_fast_last_stage.txt"
$logPath = Join-Path $modDir "treasure_static_fast.log"
...
function Stage([string]$Name, [string]$Details) {
    $text = @(
        "DragonSword Treasure static PAK probe",
        "Version: 1.0.33-treasure-static-fast",
...
try {
    ...
    try {
        ...
        function Rank-Candidate($Entry) {
            if ([string]$Entry.File -ieq 'PropTreasureBoxData.xml') { return 0 }
            if ([string]$Entry.File -ieq 'SectionTreasureBoxData.xml') { return 1 }
            return 100
        }
        ...
        $candidates = @(
            $entries |
                Where-Object {
                    ([string]$_.File -ieq 'PropTreasureBoxData.xml') -or
                    ([string]$_.File -ieq 'SectionTreasureBoxData.xml')
                } |
        ...
        if ($candidates.Count -ne 2) {
            throw ("Expected exactly 2 Treasure static candidates, found " + $candidates.Count)
        }
