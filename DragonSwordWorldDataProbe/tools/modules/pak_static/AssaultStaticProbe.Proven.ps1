...
function Stage([string]$Name, [string]$Details) {
    $text = @(
        ...
        "Version: 1.0.10-xor16-assault-runtime-table",
    ...
    [string]$ExpectedFullName)
{
    try {
        ...
        if ($NextEncodedOffset -le $EncodedOffset -or
            ($NextEncodedOffset - $EncodedOffset) -ne 16)
        {
            return $null
        }
        ...
        # Evidence for Kind/Place/Respawn shows byte #1 is the per-record XOR
        # mask because the decoded one-block compact bitfield has byte #1 == 0.
        # This is a deterministic v101 one-block decoder, not a mask search.
        [byte]$mask = $raw[1]
        ...
        [bool]$encrypted = ($bits -band (1u -shl 22)) -ne 0
        [uint32]$blockCount = ($bits -shr 6) -band 0xFFFF
        [uint32]$blockSizeCode = $bits -band 0x3F
        [bool]$offset32 = ($bits -band (1u -shl 31)) -ne 0
        [bool]$uncompressed32 = ($bits -band (1u -shl 30)) -ne 0
        [bool]$compressed32 = ($bits -band (1u -shl 29)) -ne 0

        # Narrowly accept the exact layout observed in the three blocked records:
        # Oodle slot 0, one block, no encryption, all sizes/offsets stored as u32.
        ...
        # 16 bytes means there is no extra block-size u32 in the compact record.
        ...
        [uint32]$dataCompression = Get-Value $data 'CompressionIndex'
        [byte]$dataFlags = Get-Value $data 'Flags'

        if ($dataCompressed -ne $compressedSize -or
            $dataUncompressed -ne $uncompressedSize -or
            $dataCompression -ne 1 -or
            (($dataFlags -band 1) -ne 0))
        {
            return $null
        }

        # The old compact validator rejects blockSizeCode==0 even for a one-block
        # entry. The full DataEntry is authoritative for decompression, so after
        # the exact size/compression cross-check above we write using that header.
        $a = New-Args 6
        $a[0] = $Binary
        $a[1] = $data
        $a[2] = $AesKey
...
try {
    ...
    try {
        ...
        $patterns = @(
            ...
            'UnexpectedMission',
            '(MiniGame|Mini_Game|Minigame)Data',
            '(Mole|Gopher|Groundhog|Marmot|Whack|Rodent|Burrow)',
            'Warrior',
            'Champion',
            'Assault',
        ...
        function Rank-Candidate($Entry) {
            ...
            if ($name -match '(MiniGame|Mini_Game|Minigame)Data') { return 0 }
            if ($name -match '(Mole|Gopher|Groundhog|Marmot|Whack)') { return 1 }
        ...
        if ($candidates.Count -gt 320) { $candidates = @($candidates | Select-Object -First 320) }
            ...
            try {
                ...
                try {
                    ...
                    if ($null -ne $fallback) {
                        ...
                        # Keep the already-proven legacy fallback only for support
                        # tables. Critical Kind/Place/Respawn entries never use the
                        # broad 0..255 search.
