...
$outputDir = Join-Path $modDir "kind_place_fast_probe"
$rawDir = Join-Path $outputDir "xml"
$tempDir = Join-Path ([IO.Path]::GetTempPath()) ("DSKindPlaceFast-" + [Guid]::NewGuid().ToString("N"))
$stagePath = Join-Path $modDir "kind_place_fast_last_stage.txt"
$logPath = Join-Path $modDir "kind_place_fast.log"
...
try {
    ...
    try {
        ...
        function Rank-Candidate($Entry) {
            if ([string]$Entry.File -ieq 'UnexpectedMissionPlaceData.xml') { return 0 }
            if ([string]$Entry.File -ieq 'UnexpectedMissionKindData.xml') { return 1 }
            return 100
        }
        ...
        $candidates = @(
            $entries |
                Where-Object {
                    ([string]$_.File -ieq 'UnexpectedMissionPlaceData.xml') -or
                    ([string]$_.File -ieq 'UnexpectedMissionKindData.xml')
                } |
                Sort-Object @{Expression={Rank-Candidate $_}}, FullName
        )
        if ($candidates.Count -gt 2) { $candidates = @($candidates | Select-Object -First 2) }

        Stage "EXTRACT_XML" ("candidates=" + $candidates.Count)
        if ($candidates.Count -ne 2) {
            throw ("Expected exactly 2 Kind/Place candidates, found " + $candidates.Count)
        }
