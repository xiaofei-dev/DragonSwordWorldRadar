...
function Get-Expected-TableTokens([string]$FullName)
{
    $leaf = [IO.Path]::GetFileNameWithoutExtension($FullName)
    $tokens = New-Object Collections.Generic.List[string]
    if (-not [string]::IsNullOrWhiteSpace($leaf)) {
        [void]$tokens.Add($leaf)
        if ($leaf.EndsWith('Data')) {
            [void]$tokens.Add($leaf.Substring(0, $leaf.Length - 4))
        }
    }
    return $tokens
}

function Test-XmlMatchesTarget([string]$XmlPath, [string]$ExpectedFullName)
{
    ...
    try {
        ...
        [xml]$xml = $text
        $tokens = Get-Expected-TableTokens $ExpectedFullName

        # Check document text and common schema/name attributes. GeneratedGameData
        # XML normally embeds its table or row-struct identity somewhere near the root.
        $probe = New-Object Text.StringBuilder
        [void]$probe.AppendLine($text.Substring(0, [Math]::Min($text.Length, 32768)))
        if ($null -ne $xml.DocumentElement) {
            [void]$probe.AppendLine($xml.DocumentElement.Name)
            foreach ($attr in $xml.DocumentElement.Attributes) {
                [void]$probe.AppendLine([string]$attr.Name)
                [void]$probe.AppendLine([string]$attr.Value)
            }
        }
        $haystack = $probe.ToString()

        foreach ($token in $tokens) {
            if (-not [string]::IsNullOrWhiteSpace($token) -and
                $haystack.IndexOf($token, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
                return $true
            }
        }
...
function Try-Extract-EncodedEntryByMask(
    $ProgramType,
    [byte[]]$EncodedEntries,
    [int]$EncodedOffset,
    $Binary,
...
{
    # Do not inspect adjacent directory records. That caused false positives where
    # one XML was silently substituted for its neighbor.
    [byte[]]$offsetBytes = [BitConverter]::GetBytes([int]$EncodedOffset)
    [uint32]$unsignedOffset = [BitConverter]::ToUInt32($offsetBytes, 0)
    [int]$candidateOffset = [int]($unsignedOffset -band 0x7FFFFFFF)

    if ($candidateOffset -lt 0 -or $candidateOffset -ge $EncodedEntries.Length) {
        return $null
    }

    for ($maskValue = 0; $maskValue -le 255; $maskValue++) {
        $candidateOutput = $OutputPath + ".mask-" + $candidateOffset + "-" + $maskValue + ".tmp"
        try {
            $a = New-Args 3
            $a[0] = $EncodedEntries
            ...
            if ($uncompressedSize -eq 0 -or $uncompressedSize -gt 268435456) { continue }
            if ($compressedSize -eq 0 -or $compressedSize -gt 268435456) { continue }
            if ($compressionSlot -lt -1 -or $compressionSlot -gt 63) { continue }
            if ($blockCount -gt 65536) { continue }
            ...
            if (-not [IO.File]::Exists($candidateOutput)) { continue }
            [byte[]]$bytes = [IO.File]::ReadAllBytes($candidateOutput)
            if (-not (Test-LooksLikeXml $bytes)) { continue }
            if (-not (Test-XmlMatchesTarget $candidateOutput $ExpectedFullName)) { continue }

            Move-Item -LiteralPath $candidateOutput -Destination $OutputPath -Force
            return [pscustomobject]@{
                Mode = 'MASK_RECOVERY_VALIDATED'
...
function Try-Extract-SmallUncompressedVariants(
    [byte[]]$EncodedEntries,
    [int]$EncodedOffset,
    $Binary,
    [string]$OutputPath,
    [string]$ExpectedFullName)
{
    [byte[]]$offsetBytes = [BitConverter]::GetBytes([int]$EncodedOffset)
    [uint32]$unsignedOffset = [BitConverter]::ToUInt32($offsetBytes, 0)
    [int]$baseOffset = [int]($unsignedOffset -band 0x7FFFFFFF)

    # Candidate directory-record layouts seen in small GeneratedGameData entries.
    # Stay on the same directory record; do not move to neighboring records.
    $layouts = @(
        @{ OffsetPos = 0; SizePos = 8; OffsetBytes = 8; SizeBytes = 8; Header = 0 },
        @{ OffsetPos = 0; SizePos = 8; OffsetBytes = 8; SizeBytes = 4; Header = 0 },
        @{ OffsetPos = 0; SizePos = 4; OffsetBytes = 4; SizeBytes = 4; Header = 0 },
        @{ OffsetPos = 4; SizePos = 12; OffsetBytes = 8; SizeBytes = 4; Header = 0 },
        @{ OffsetPos = 0; SizePos = 8; OffsetBytes = 8; SizeBytes = 8; Header = 16 },
        @{ OffsetPos = 0; SizePos = 8; OffsetBytes = 8; SizeBytes = 4; Header = 16 },
        @{ OffsetPos = 0; SizePos = 4; OffsetBytes = 4; SizeBytes = 4; Header = 16 },
        @{ OffsetPos = 0; SizePos = 8; OffsetBytes = 8; SizeBytes = 8; Header = 32 }
    )

    foreach ($layout in $layouts) {
        try {
            $maxIndex = [Math]::Max(
                [int]$layout.OffsetPos + [int]$layout.OffsetBytes,
                [int]$layout.SizePos + [int]$layout.SizeBytes
            )
            if ($baseOffset -lt 0 -or $baseOffset + $maxIndex -gt $EncodedEntries.Length) {
                continue
            }
            ...
            if ([int]$layout.OffsetBytes -eq 8) {
                $pakOffset = [BitConverter]::ToUInt64($EncodedEntries, $baseOffset + [int]$layout.OffsetPos)
            }
            else {
                $pakOffset = [BitConverter]::ToUInt32($EncodedEntries, $baseOffset + [int]$layout.OffsetPos)
            }

            if ([int]$layout.SizeBytes -eq 8) {
                $size = [BitConverter]::ToUInt64($EncodedEntries, $baseOffset + [int]$layout.SizePos)
            }
            else {
                $size = [BitConverter]::ToUInt32($EncodedEntries, $baseOffset + [int]$layout.SizePos)
            }

            $pakOffset = $pakOffset -band 0x7FFFFFFFFFFFFFFF
            if ($size -eq 0 -or $size -gt 16777216) { continue }
            if ($pakOffset + [uint64]$layout.Header + $size -gt [uint64]$Binary.BaseStream.Length) { continue }

            [void]$Binary.BaseStream.Seek(
                [int64]($pakOffset + [uint64]$layout.Header),
                [IO.SeekOrigin]::Begin
            )
            [byte[]]$bytes = $Binary.ReadBytes([int]$size)
            if ($bytes.Length -ne [int]$size) { continue }
            if (-not (Test-LooksLikeXml $bytes)) { continue }

            [IO.File]::WriteAllBytes($OutputPath, $bytes)
            if (-not (Test-XmlMatchesTarget $OutputPath $ExpectedFullName)) {
                Remove-Item -LiteralPath $OutputPath -Force -ErrorAction SilentlyContinue
                continue
            }

            return [pscustomobject]@{
                Mode = 'SMALL_UNCOMPRESSED_VALIDATED'
                EncodedOffset = $baseOffset
                PakOffset = $pakOffset
                Size = $size
                HeaderSkip = [int]$layout.Header
                Layout =
                    'offset' + $layout.OffsetPos + 'x' + $layout.OffsetBytes +
                    '_size' + $layout.SizePos + 'x' + $layout.SizeBytes
            }
        }
        catch {
            Remove-Item -LiteralPath $OutputPath -Force -ErrorAction SilentlyContinue
        ...
        foreach ($entry in $candidates) {
            ...
            try {
                ...
                try {
                    ...
                    $fallback = Try-Extract-EncodedEntryByMask `
                        $programType `
                        $encodedEntries `
                    ...
                    if ($null -eq $fallback) {
                        $fallback = Try-Extract-SmallUncompressedVariants `
                            $encodedEntries `
                            ([int]$entry.EncodedOffset) `
                            $binary `
                            $outputPath `
                    ...
                    if (-not (Test-XmlMatchesTarget $outputPath ([string]$entry.FullName))) {
                        Remove-Item -LiteralPath $outputPath -Force -ErrorAction SilentlyContinue
                        throw ($primaryError + ' | recovered XML identity did not match target entry')
                    }
                ...
                $recoveryDetail = ''
                if ($null -ne $fallback -and $mode -like 'MASK_RECOVERY*') {
                    $recoveryDetail =
                        ' | recoveredOffset=' + $fallback.EncodedOffset +
                ...
                $manifest.Add(
                    ...
                    ' | ' + $entry.FullName +
                    ' | ' + $outputName +
                    ' | encodedOffset=' + $entry.EncodedOffset +
                    $recoveryDetail
                )
