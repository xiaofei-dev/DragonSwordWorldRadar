...
local settings = config.module_settings and config.module_settings.ui_snapshot or {}
        ...
        if type(target) == "table" and type(target.class) == "string" and target.class ~= "" then
            steps[#steps + 1] = {
                ...
                run = function(ctx)
                    ...
                    local limit = math.max(1, tonumber(target.max_objects or (config.module_settings.ui_snapshot and config.module_settings.ui_snapshot.default_max_objects)) or 64)
                    local fields = target.fields or target.fields or { "ID", "CID", "State", "Status", "bCompleted" }
                    ...
                    return {
                        status="readable", value_type="ui_snapshot",
...
return { id="ui_snapshot", enabled=#steps > 0, steps=steps }
