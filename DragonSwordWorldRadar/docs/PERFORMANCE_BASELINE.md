...
## Current candidate: 0.4.0-dev74-processdispatchguard1-localcapfix1
...
The current candidate preserves the dev72 compact cadence and every visible layer. It retains the 250 ms fresh-current-Pawn sample, 50 ms scalar-only compact flush, 33 ms compact Overlay presentation, 8 ms expanded sampling/presentation, epoch/token travel safety, and direct-vector rendering. Runtime evidence invalidated the persistent EngineTick action: UE4SS reported `Ref was not function`, after which expanded production and F7 callbacks stopped. Later runtime evidence showed ProcessEvent accepting activation requests without invoking their callbacks. WorldRadar therefore serializes activation, compact control, the one-shot clock, and expanded-map production through one stable ProcessEvent callback and one shared in-flight slot. Producers coalesce instead of creating independent UE4SS actions, and task plus dispatcher epochs reject stale work before UObject access. A callback that fails to enter within three seconds poisons ProcessEvent once for the session and disables WorldRadar without retry or EngineTick fallback. This adds only scalar arbitration on existing loops and does not add a callback, timer, UObject read, bridge read, draw, or save operation. The two nearest treasure candidates remain in the existing selection pass; the second adds only one normal vector marker and optional height arrow. F5/F6 hotkeys are registered while `debug_logging = true`, which is the current acceptance default.
...
Save snapshots use one non-harmonic 19-second change-check contract. An unchanged active-slot/key fingerprint exits before copy or SQLCipher; a changed source queues one complete snapshot immediately and must remain identical across its copy. Successful and failed workers start a fresh 19-second completion cooldown, eliminating immediate catch-up SQL after a long query; pending changes remain remembered and are revalidated before the next eligible queue. The worker enters balanced Windows background mode, ignores zero-only treasure categories, and skips the diagnostic-only `tb_unexpected_switch_week` query when debug logging is disabled. With acceptance debug enabled, the large treasure/filter detail block is generated only when save/catalog/index versions change instead of every two seconds while coordinates move. These changes do not remove Treasure, Boss, Assault, Mole/Fly, world-status, compact-radar, or expanded-map output. They are source-verified optimizations; only an in-game A/B can establish the frame-time effect.
...
Assault reuses the existing `tb_actor_respawn` snapshot and shared Radar DAILY policy. The observed recovery boundary is 09:00 Korea Standard Time (00:00 UTC), independent of the player's local time zone. Save state is checked every nineteen seconds only while F7 is active. An unchanged active-slot/key fingerprint ends the tick without SQLCipher; a changed source queues the low-priority worker immediately and must remain identical across its copy. Parsed `.db` and `.bak` results are cached independently by exact session fingerprint; each cache miss builds one Treasure-rich entry for all layers. No additional cross-restart cache or reader is introduced.
...
- Once-per-game-frame active world-map transform production with 8 ms scalar bridge/presentation, paused immediately when the map closes.
...
- `high_resolution_timer = true` is the acceptance default and requests `timeBeginPeriod(1)`; `false` uses the Windows default outside the expanded map. Restart the game between samples.
