...
- Active world-map transform production and Overlay presentation: 8 ms, only while the map is visible; both reuse the shared 250 ms numeric player sample and stop immediately on exit. This restores the pre-diagnostic cadence after the 4 ms runtime A/B. A balanced mode-scoped timer-resolution request is held only while the expanded map is visible and is not double-acquired when the startup-wide user option is enabled.
