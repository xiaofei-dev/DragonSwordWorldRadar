...
if ($overlaySources.Count -ne 42) {
    throw "Overlay source set must contain exactly 42 files; found $($overlaySources.Count)."
}
$overlayReferences = @(
...
$installerSources = @(Get-ChildItem -LiteralPath (Join-Path $root 'src\installer\Core') -Recurse -Filter '*.cs' -File |
    Where-Object { $_.FullName -notmatch '[\\/](?:obj|bin)[\\/]' } |
    Sort-Object FullName | Select-Object -ExpandProperty FullName)
if ($installerSources.Count -ne 20) {
    throw "Installer source set must contain exactly 20 files; found $($installerSources.Count)."
}
$installerReferences = @(
