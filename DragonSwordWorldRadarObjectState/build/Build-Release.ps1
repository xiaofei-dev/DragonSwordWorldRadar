...
Invoke-Gate 'Test-TreasureActorCatalog.ps1'
$nativeSdk = Join-Path (Split-Path -Parent $root) `
    'DragonSwordNativeAutoPickup\.sdk\RE-UE4SS'
& (Join-Path $root 'tools\Build-Native.ps1') -UE4SSRoot $nativeSdk
if ($LASTEXITCODE -ne 0) {
    throw "Native provider build failed (exit=$LASTEXITCODE)"
}
...
$stage = Join-Path $dist ("DragonSwordWorldRadarObjectState-$version")
...
Copy-One (Join-Path $root 'build-native\main.dll') (Join-Path $mod 'dlls\main.dll')
...
$archive = Join-Path $dist ("DragonSwordWorldRadarObjectState-v$version.zip")
