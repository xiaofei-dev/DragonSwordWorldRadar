...
namespace DragonSwordWorldRadar
{
    internal static class AssaultIntegrationHarness
    {
        ...
        {string policy=Path.Combine(root,name+"-policy.xml"),output=Path.Combine(root,name+".lua");File.WriteAllText(policy,policyText);Assert(AssaultLuaGenerator.Generate(place,kind,monster,reveal,policy,fingerprint,output)==40,name+" generator count");List<KeyValuePair<string,string>> mappings=new List<KeyValuePair<string,string>>();foreach(Match match in Regex.Matches(File.ReadAllText(output),@"\bplace_id\s*=\s*(\d+)[^\r\n]*\bcid\s*=\s*(\d+)"))mappings.Add(new KeyValuePair<string,string>(match.Groups[1].Value,match.Groups[2].Value));string actors=Path.Combine(root,name+"-actors.tsv");Assert(EncounterActorCatalogGenerator.GenerateMappings(monster,Path.Combine(root,"character.xml"),mappings,actors)==40,name+" actor generator count");Assert((File.ReadAllLines(actors)[1].Split('\t')[0])=="101",name+" actor event ID uses place_id");return output;}
        ...
        {
            StringBuilder p=new StringBuilder("<Rows>"),k=new StringBuilder("<Rows>"),m=new StringBuilder("<Rows>"),c=new StringBuilder("<Rows>");
            for(int i=1;i<=40;i++){int placeId=100+i,cid=i==4?143:999+i;string uid=i==1?"17806813628890497727":(i==4?"5945914773662957327":(8000000000000000000UL+(ulong)i).ToString(CultureInfo.InvariantCulture));string name=i==4?"DSkeletonLeader_Nam_1041101":"Name"+i;
                ...
                c.AppendFormat(CultureInfo.InvariantCulture,"<Row ID=\"{0}\" BluePrint=\"/Game/Test/Monster{0}.Monster{0}_C\" />",cid);}
            ...
            File.WriteAllText(Path.Combine(Path.GetDirectoryName(place),"character.xml"),c.Append("</Rows>").ToString());
