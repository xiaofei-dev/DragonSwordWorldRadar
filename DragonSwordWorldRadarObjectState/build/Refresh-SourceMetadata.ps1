...
$stageRoot = Join-Path $root ("dist\DragonSwordWorldRadarObjectState-$version\DragonSwordWorldRadarObjectState")
...
Add-Mapping $mappings 'build-native\main.dll' 'dlls\main.dll'
...
$sourceSnapshot = [ordered]@{
    ...
    runtime_ipc = 'native protocol-v1 seqlock shared memory for coordinates/events plus protocol-v6 Lua control/status/world-map geometry files'
    native_shared_memory_protocol = 1
    native_shared_memory_bytes = 5248
...
foreach ($file in @(Get-ChildItem -LiteralPath $root -Recurse -File | Sort-Object FullName)) {
    ...
    if (($relative -eq 'metadata/source-manifest.json') -or
        ...
        $relative.StartsWith('build-native/', [StringComparison]::OrdinalIgnoreCase) -or
        $relative.StartsWith('build-native-core/', [StringComparison]::OrdinalIgnoreCase) -or
...
$manifest = [ordered]@{
    ...
    excludes = @('metadata/source-manifest.json','.git/','dist/','build-native/','build-native-core/','runtime/','src/overlay/obj/','src/overlay/bin/','src/installer/Core/obj/','src/installer/Core/bin/')
    baseline_commit = $baselineCommit.Trim()
    scope = 'complete DragonSwordWorldRadarObjectState repository source inventory'
