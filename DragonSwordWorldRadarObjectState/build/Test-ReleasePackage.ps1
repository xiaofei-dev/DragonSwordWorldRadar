...
$stage = Join-Path $root ("dist\DragonSwordWorldRadarObjectState-$Version")
$mod = Join-Path $stage 'DragonSwordWorldRadarObjectState'
$archive = Join-Path $root ("dist\DragonSwordWorldRadarObjectState-v$Version.zip")
...
if ($libraries.Count -ne 2 -or
    @($libraries | Where-Object {
        ($_ -ieq 'vendor/sqlcipher/e_sqlcipher.dll') -or
            ($_ -ieq 'dlls/main.dll')
    }).Count -ne 2) {
    throw 'Unexpected DLL set in release staging.'
}
$nativeDll = Join-Path $mod 'dlls\main.dll'
if (-not (Test-Path -LiteralPath $nativeDll -PathType Leaf) -or
    (Get-Item -LiteralPath $nativeDll).Length -lt 100000) {
    throw 'Native UE4SS provider DLL is missing or unexpectedly small.'
}
...
try {
    $expectedPrefix = 'DragonSwordWorldRadarObjectState/'
    $expectedDirectories = @{}
    foreach ($requiredDirectory in @(
        'DragonSwordWorldRadarObjectState/data/generated/',
        'DragonSwordWorldRadarObjectState/runtime/bridge/',
        'DragonSwordWorldRadarObjectState/runtime/diagnostics/',
        'DragonSwordWorldRadarObjectState/runtime/logs/')) {
