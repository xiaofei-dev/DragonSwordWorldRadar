...
namespace DragonSwordWorldRadar
{
    internal sealed class TreasureSaveState
    {
        ...
        private readonly HashSet<long> _nativeOpened =
            new HashSet<long>();
        private readonly Dictionary<int, DateTime>
            _nativeEncounterDefeated =
                new Dictionary<int, DateTime>();
        private int _nativeActivation;
        private int _syncAttemptedActivation;
        private bool _activationSyncPending;
        ...
        public void SetNativeActivation(
            int activation,
            bool enabled)
        {
            if (activation <= 0)
            ...
            lock (_sync)
            {
                if (_nativeActivation == activation)
                {
                    return;
                }
                _nativeActivation = activation;
                _nativeOpened.Clear();
                _nativeEncounterDefeated.Clear();
                _activationSyncPending = enabled;
                _syncAttemptedActivation = 0;
                _loadToken++;
                _loadInProgress = false;
            }
            ErrorLog.WriteDebug(String.Format(
                CultureInfo.InvariantCulture,
                "NATIVE_ACTIVATION activation={0}; enabled={1}; sqlSyncQueued={2}; nativeDeltasReset=true",
                activation,
                enabled ? 1 : 0,
                enabled ? 1 : 0));
        }

        public bool MarkOpenedFromNative(
            long saveId,
            int activation)
        {
            if (saveId <= 0)
            ...
            long categoryValue = saveId / 64;
            if (categoryValue > Int32.MaxValue)
            {
                return false;
            }
            int category = (int)categoryValue;
            int bit = (int)(saveId % 64);
            lock (_sync)
            {
                if (!_runtimeEnabled
                    || activation != _nativeActivation
                    || !_nativeOpened.Add(saveId))
                {
                    return false;
                }
                Dictionary<int, ulong> opened =
                    new Dictionary<int, ulong>(_opened);
                ulong field;
                opened.TryGetValue(category, out field);
                ulong updated = field | (1UL << bit);
                opened[category] = updated;
                _opened = opened;
        ...
        public bool MarkEncounterDefeatedFromNative(
            int encounterId,
            ...
            int activation)
        {
            ...
            DateTime normalized = destroyTimeUtc.Kind == DateTimeKind.Utc
                ? destroyTimeUtc
            ...
            lock (_sync)
            {
                ...
                DateTime existingNative;
                if (_nativeEncounterDefeated.TryGetValue(
                        encounterId,
                        out existingNative)
                    && existingNative >= normalized)
                {
                    return false;
                }
                _nativeEncounterDefeated[encounterId] = normalized;
                Dictionary<int, BossRespawnRecord> respawns =
                    new Dictionary<int, BossRespawnRecord>(_bossRespawns);
                BossRespawnRecord existing;
                if (!respawns.TryGetValue(encounterId, out existing)
                    || existing.DestroyTimeUtc < normalized)
                {
                    respawns[encounterId] = new BossRespawnRecord
                    {
                        BossId = encounterId,
                        RespawnType = 0,
                        DestroyTimeUtc = normalized
                    };
                    _bossRespawns = respawns;
                    _version++;
                    return true;
                }
                return false;
        ...
        public void SetRuntimeEnabled(bool enabled)
        {
            ...
            lock (_sync)
            {
                ...
                if (enabled)
                {
                    if (_nativeActivation > 0
                        && _syncAttemptedActivation
                            != _nativeActivation)
                    {
                        _activationSyncPending = true;
                    }
                }
                else
                {
        ...
        public void Refresh()
        {
            ...
            {
                if (!_runtimeEnabled
                    || !_activationSyncPending
                    || _loadInProgress
                    || _nativeActivation <= 0)
                {
                    return;
                }
                // Consume before touching the process or save. A failed F7
                // reconciliation is deliberately not retried until the user
                // starts a new activation, preventing an unbounded error loop.
                activation = _nativeActivation;
                _activationSyncPending = false;
            ...
            try
            {
                    ...
                    GameProcessFinder.OpenTracked(_gameProcessId))
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "F7 save reconciliation could not find the game process.");
                    }
                    ...
                    RefreshFromGame(game, activation);
                }
            }
            ...
            try
            {
                lock (_sync)
                {
                    if (!_runtimeEnabled
                        ...
                        || _nativeActivation != request.Activation)
                    {
                        return;
                    }
                }
                ...
                lock (_sync)
                {
                    ...
                    opened = treasureRefreshAvailable
                        ? new Dictionary<int, ulong>(snapshot.Opened)
                        : new Dictionary<int, ulong>(previousOpened);
                ...
                if (DebugSettings.Enabled)
                {
                    ErrorLog.WriteDebug(String.Format(
                        CultureInfo.InvariantCulture,
                        "F7_SAVE_SYNC_PERF activation={0}; totalMs={1:F3}; copyMs={2:F3}; keyMs={3:F3}; treasureQueryMs={4:F3}; bossQueryMs={5:F3}; encounterTaskQueryMs={6:F3}; databaseReads={7}; databaseCacheHits={8}; treasureRequested={9}; treasureQueries={10}; retryPolicy=next-activation-only",
                        request.Activation,
        ...
        private void MergeNativeOpened(
            IDictionary<int, ulong> opened)
        {
            foreach (long saveId in _nativeOpened)
            {
                int category = (int)(saveId / 64);
                int bit = (int)(saveId % 64);
                ulong field;
                opened.TryGetValue(category, out field);
                opened[category] = field | (1UL << bit);
            }
        }

        private void MergeNativeEncounterDefeats(
            IDictionary<int, BossRespawnRecord> respawns)
        {
            foreach (KeyValuePair<int, DateTime> pair
                in _nativeEncounterDefeated)
            {
                BossRespawnRecord existing;
                if (!respawns.TryGetValue(pair.Key, out existing)
                    || existing.DestroyTimeUtc < pair.Value)
                {
                    respawns[pair.Key] = new BossRespawnRecord
                    {
                        BossId = pair.Key,
                        RespawnType = 0,
                        DestroyTimeUtc = pair.Value
                    };
                }
            }
        }
