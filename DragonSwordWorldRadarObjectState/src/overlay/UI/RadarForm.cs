...
namespace DragonSwordWorldRadar
{
    internal sealed class RadarForm : Form
    {
        ...
        private readonly NativeStateBridgeReader _nativeStateBridge;
        private readonly List<NativeStateEvent> _nativeStateEvents =
            new List<NativeStateEvent>(16);
        ...
        private long _lastNativeSampleSequence = -1;
        private int _lastNativeActivation;
        ...
        public RadarForm()
        {
            ...
            _nativeStateBridge = new NativeStateBridgeReader();
            ...
            _trayIcon = new NotifyIcon
            {
                ...
                Text = "DragonSwordWorldRadarObjectState",
        ...
        private ContextMenuStrip CreateTrayMenu()
        {
            ...
            ToolStripMenuItem exitItem = new ToolStripMenuItem(
                "Exit DragonSwordWorldRadarObjectState");
        ...
        private static bool NativePositionUsable(
            NativeStateFrame state)
        {
            return state != null
                && state.Enabled
                && state.PositionValid
                && !state.TransitionActive
                && !Double.IsNaN(state.PlayerX)
                && !Double.IsInfinity(state.PlayerX)
                && !Double.IsNaN(state.PlayerY)
                && !Double.IsInfinity(state.PlayerY)
                && !Double.IsNaN(state.PlayerZ)
                && !Double.IsInfinity(state.PlayerZ)
                && state.SampleTimestampMilliseconds >= 0;
        }

        private static void ApplyNativePosition(
            MotionFrame motion,
            NativeStateFrame state)
        {
            motion.PlayerX = state.PlayerX;
            motion.PlayerY = state.PlayerY;
            motion.PlayerZ = state.PlayerZ;
            motion.HasPlayerZ = true;
            motion.SampleTimestampMs =
                state.SampleTimestampMilliseconds;
        }
        ...
        private void RefreshState()
        {
            ...
            NativeStateFrame nativeState = null;
            long missedNativeEvents = 0;
            _nativeStateEvents.Clear();
            bool nativeStateRead = false;
            try
            {
                nativeStateRead = _nativeStateBridge.TryRead(
                    _gameProcessId,
                    _nativeStateEvents,
                    out nativeState,
                    out missedNativeEvents);
                if (nativeStateRead
                    && nativeState.Activation > 0
                    && nativeState.Activation != _lastNativeActivation)
                {
                    _lastNativeActivation = nativeState.Activation;
                    _lastNativeSampleSequence = -1;
                    _saveState.SetNativeActivation(
                        nativeState.Activation,
                        nativeState.Enabled);
                }
                ClearModuleFailure("native-state-bridge");
            }
            catch (Exception exception)
            {
                ReportModuleFailure(
            ...
            if (_motionBridge.TryReadLatest(
                ...
                out motion))
            {
                if (NativePositionUsable(nativeState))
                {
                    ApplyNativePosition(motion, nativeState);
                }
            ...
            if (NativePositionUsable(nativeState)
                && _motion != null
                && nativeState.SampleSequence
                    != _lastNativeSampleSequence
                && _diagnosticMode != DiagnosticModeNoMotion)
            {
            ...
            _saveState.SetRuntimeEnabled(
                activeMotion != null
                    ...
                    && (!nativeStateRead || nativeState.Enabled));
            if (nativeStateRead)
            {
                if (missedNativeEvents > 0)
                {
                    ErrorLog.WriteMessage(
                        String.Format(
                            CultureInfo.InvariantCulture,
                            "Native object-state event overflow: missed={0}; recovery=next-F7-save-sync",
                            missedNativeEvents));
                }
                for (int index = 0;
                    index < _nativeStateEvents.Count;
                    index++)
                {
                    NativeStateEvent item = _nativeStateEvents[index];
                    if (item.Kind
                        == NativeStateEventKind.TreasureOpened)
                    {
                        redraw |= _saveState.MarkOpenedFromNative(
                            item.Id,
                            item.Activation);
                    }
                    else if (item.Kind
                        == NativeStateEventKind.EncounterDefeated)
                    {
                        DateTime destroyedUtc;
                        try
                        {
                            destroyedUtc = DateTimeOffset
        ...
        private void CheckGameLifetime()
        {
            ...
            ErrorLog.WriteDebug(
                "Game process exited; closing DragonSwordWorldRadarObjectState overlay.");
            Close();
        }
