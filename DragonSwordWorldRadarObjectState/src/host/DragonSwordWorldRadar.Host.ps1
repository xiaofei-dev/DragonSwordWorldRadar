...
$log = Join-Path $logDir 'DragonSwordWorldRadarObjectState.Host.log'
...
try {
    $createdNew = $false
    $mutex = New-Object Threading.Mutex($true, 'Local\DragonSwordWorldRadarObjectState.OverlayHost', [ref]$createdNew)
    if (-not $createdNew) { Log 'ALREADY_RUNNING'; return }
    $ownsMutex = $true
    Log "START version=0.5.0-dev2-native-presence renderer=WinForms pid=$PID host=single_process_watcher"
...
'@
    ...
    [IO.File]::WriteAllText(
        ...
        '0.5.0-dev2-native-presence WinForms',
