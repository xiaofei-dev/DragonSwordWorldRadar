...
$processWorkDir = Join-Path ([IO.Path]::GetTempPath()) 'DragonSwordWorldRadarObjectState'
...
$log = Join-Path $logDir 'DragonSwordWorldRadarObjectState.WatcherHost.log'
...
function Test-RadarEnabled {
    ...
    try {
        ...
        $match = [regex]::Match(
            $modsText,
            '(?m)^[ \t]*DragonSwordWorldRadarObjectState[ \t]*:[ \t]*([01])')
        if ($match.Success) {
...
$createdNew = $false
$mutex = New-Object Threading.Mutex($true, 'Local\DragonSwordWorldRadarObjectState.TransientHost', [ref]$createdNew)
if (-not $createdNew) { Log "HOST_ALREADY_RUNNING stamp=$RequestStamp pid=$PID"; exit 0 }
try {
Log "HOST_START version=0.5.0-dev2-native-presence stamp=$RequestStamp pid=$PID"
    if (-not (Test-RadarEnabled)) {
        Log "MOD_DISABLED DragonSwordWorldRadarObjectState=0 stamp=$RequestStamp"
        exit 0
    }
    ...
    if (-not $compatibility.Compatible) {
        ...
        [System.Windows.Forms.MessageBox]::Show(
            "DragonSwordWorldRadarObjectState data was generated for game version: $installedVersion`r`nCurrent game version: $($compatibility.Current.display_version)`r`n`r`nRun Install.cmd to regenerate local game data.",
            'DragonSwordWorldRadarObjectState - Reinstall required',
