    ...
    if type(source) == "string" and string.sub(source, 1, 1) == "@" then
        ...
        if mod_directory ~= nil then
            ...
            use_log_path = log_directory
                .. "\\DragonSwordWorldRadarObjectState.Lua.Use.log"
            debug_log_path = log_directory
                .. "\\DragonSwordWorldRadarObjectState.Lua.Debug.log"
        end
    end
    return use_log_path, debug_log_path
end
...
function Diagnostics.configure(options)
    ...
    if not configured then
        if use_enabled then
            prepare_log(
                use_path,
                "DragonSwordWorldRadarObjectState.Lua.Use"
            )
        end
        if debug_enabled then
            prepare_log(
                debug_path,
                "DragonSwordWorldRadarObjectState.Lua.Debug"
            )
        end
    end
    configured = true
