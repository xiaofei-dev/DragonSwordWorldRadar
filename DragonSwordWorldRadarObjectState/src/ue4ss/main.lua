local MOD = "[DragonSwordWorldRadarObjectState]"
...
local GENERATION_KEY = "DragonSwordWorldRadarObjectState.Generation"
...
resolve_mod_directory = function()
    ...
    if mod_directory == nil then
        ...
        if type(win64_path) == "string" then
            mod_directory = win64_path .. "\\ue4ss\\Mods\\DragonSwordWorldRadarObjectState"
        end
    end
    return mod_directory
end
...
local function start_overlay()
    ...
    if directory == nil then
        log("Could not resolve the DragonSwordWorldRadarObjectState directory; overlay request was not written.")
        return
    end
...
local function motion_has_visual_change(
    ...
    -- Native shared memory owns compact player motion. The file bridge only
    -- carries controls, status, heartbeat, and world-map geometry; player
    -- movement must not cause filesystem writes.
